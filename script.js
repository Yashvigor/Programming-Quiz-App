...
function loadQuestion() {
  ...
  document.getElementById("question-count").textContent = `Question ${currentQuestionIndex + 1} of ${questions[currentCategory].length}`;
...
function endQuiz() {
  ...
  document.getElementById("score-display").textContent = `Your score: ${score} / ${questions[currentCategory].length}`;
  ...
  let highScore = localStorage.getItem(currentCategory + "-highscore") || 0;
  if (score > highScore) {
    highScore = score;
    localStorage.setItem(currentCategory + "-highscore", highScore);
  }
...
function startTimer() {
  const FULL_DASH_ARRAY = 125.6; // 2 * π * 20 (r=20)
